...
class Seq2Seq_Class(object):
    def __init__(self, param: ParameterConfig, model_save_path: str,
    ...
    def predict(self, batch_inputs, batch_ruitu, batch_ids, batch_times):
        ...
        for i in range(self.param.num_stations):
            ...
            n_statistics = result.shape[-1].value
            n_half_of_statistics = n_statistics // 2
            var_result = result[:, :, n_half_of_statistics:n_statistics]  # Variance
            result = result[:, :, 0:n_half_of_statistics]  # Mean
        ...
        self.pred_std_result = np.sqrt(pred_var_result)
    ...
    def renorm_for_visualization(self, obs_inputs, ruitu_inputs, pred_mean_result, pred_var_result, ground_truth=None):
        ...
        for target_v in self.target_list:
            ...
            obs_inputs[:, :, :, self.obs_and_output_feature_index_map[target_v]] = renorm(temp1,
                                                                                          self.obs_range_dic[target_v][
                                                                                              0],
                                                                                          self.obs_range_dic[target_v][
                                                                                              1])
            ruitu_inputs[:, :, :, self.ruitu_feature_index_map[target_v]] = renorm(temp2,
                                                                                   self.obs_range_dic[target_v][0],
                                                                                   self.obs_range_dic[target_v][1])
            pred_mean_result[:, :, :, self.obs_and_output_feature_index_map[target_v]] = renorm(temp3,
                                                                                                self.obs_range_dic[
                                                                                                    target_v][0],
                                                                                                self.obs_range_dic[
                                                                                                    target_v][1])
            ...
            if ground_truth is not None:
                ...
                ground_truth[:, :, :, self.obs_and_output_feature_index_map[target_v]] = renorm(temp5,
                                                                                                self.obs_range_dic[
                                                                                                    target_v][0],
                                                                                                self.obs_range_dic[
                                                                                                    target_v][1])
    ...
    def renorm_for_submit(self, pred_mean, pred_var, ruitu_inputs, timestep_to_ensemble=21, alpha=1):
        ...
        assert pred_mean.shape == (10, 37,
                                   3), 'Error! This funtion ONLY works for one data sample with shape (10, 37, 3). Any data shape (None, 10, 37, 3) will leads this error!'
